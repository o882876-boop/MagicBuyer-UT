...
// @version      4.0.0-fc27fix-tc11
// @description  MagicBuyer FC27 相容修正 + 完整繁體中文 + FC27 評分範圍穩定修正
...
(() => {
  ...
  const loadTranslations = async () => {
    try {
      ...
      console.warn('[MagicBuyer TC11] 無法載入 tc5 翻譯字典', err);
    }
  };
  ...
  const patchCode = (source) => {
    ...
    const priceVarsReplacement = 'const __mbRange=(o=>{let n=0,r=0,s=new Set;const l=(o,a=0,p="")=>{if(!o||"object"!=typeof o||a>2||s.has(o))return;s.add(o);Object.keys(o).forEach((k=>{let v;try{v=o[k]}catch(e){return}const q=(p+"."+k).toLowerCase(),m=parseInt(v,10),g=/rating|overall|ovr/.test(q);g&&Number.isFinite(m)&&m>=1&&m<=99&&(/min|from|low/.test(q)&&(n=m),/max|to|high/.test(q)&&(r=m)),a<2&&v&&"object"==typeof v&&!Array.isArray(v)&&l(v,a+1,q)}))};return l(o),[n,r]})(w),__mbMin=__mbRange[0],__mbMax=__mbRange[1];__mbMin&&(e.idAbMinRating=__mbMin),__mbMax&&(e.idAbMaxRating=__mbMax);let S=_(e.idAbMaxBid),T=_(e.idAbBuyPrice);(0,c.c2)(`評分篩選：最低 ${null!=e.idAbMinRating?e.idAbMinRating:"-"} / 最高 ${null!=e.idAbMaxRating?e.idAbMaxRating:"-"}${__mbMin||__mbMax?"（EA搜尋範圍）":"（市場設定）"}`,i.idProgressAutobuyer);';
  ...
  const run = (source) => {
    try {
      ...
      console.log('[MagicBuyer FC27 TC] 已載入修正版 tc11');
      startTranslator();
    } catch (err) {
